...
def do_check_last_send_time(coffeeDao):
    ...
    for i in result:
        ...
        if not diff_time:
            is_broken = True
            alert_msg = "number:%d coffee machine did not post status,may be broken" % machine_id
        elif diff_time >= machine_max_interval:
            is_broken = True
            alert_msg = "number:%d coffee machine last time post status was %d ago,may be broken" % (machine_id,
                                                                                                   diff_time)
